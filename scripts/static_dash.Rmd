...
avg_workout_long <- get_avg_workouts_data(workouts, TRUE)
...
ggplot(plot_data, aes(x = name, y = raw_intensity_score, group = name, color = name)) + 
    geom_boxplot() + 
...
ggplot(l30d, aes(x = date, color = name)) +
    geom_point(aes(y = (raw_intensity_score * 1000))) +
    geom_line(aes(y = avg_score))
```
