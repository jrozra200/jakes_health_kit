...
mth <- tibble(miles = 1:26) %>% 
    mutate(hardness = log(miles))

ggplot(mth, aes(x = miles, y = hardness)) + 
    geom_point() + 
    geom_line() + 
...
# hardness = speed * (log(distance) + altitude)

sth <- tibble(speed = seq(4, 12, by = 0.1)) %>% 
    mutate(hardness = exp(speed),
           hardness = (hardness - min(hardness)) / (max(hardness) - min(hardness)))

summary(sth)

ggplot(sth %>% filter(speed > 4 & speed < 8), aes(x = speed, y = hardness)) + 
    geom_point() + 
    geom_line() + 
    scale_x_continuous(breaks = 1:26) + 
    theme(panel.background = element_blank(),
...
# hardness = exp(speed) * (log(distance) + altitude)

summary(runs)

runs <- runs %>% 
    mutate(perc_gain = altitude_gain / (distance * 5280))
```
...
runs <- act_df %>% 
    ...
    mutate(avg_speed_mph = distance_miles / moving_time_min * 60,
           ...
           treadmill = ifelse(treadmill > 0, 1, 0),
           incline = elevation_miles / distance_miles,
           score = exp(avg_speed_mph) * log(distance_miles)) %>% 
...
runs_who %>% 
    select(date,
           ...
           calories) %>% 
    left_join(runs) %>% 
    filter(date != as_date("2022-01-15")) %>% 
    mutate(strain__score = raw_intensity_score / score,
           strain__score = (strain__score - min(strain__score)) / 
               (max(strain__score) - min(strain__score))) %>% 
    select(date,
           raw_intensity_score,
           kilojoules,
           calories,
    ...
    ggplot(aes(x = date, y = strain__score)) + 
        geom_line(stat = "identity") + 
        geom_smooth()

ggplot(runs %>% filter(date > as_date("2022-07-01")), aes(x = date, y = score)) + 
